...
const admin = db.collection('student');//注意，这里就是刚才的集合的名字——student
  ...
  login() {
    ...
    admin.get({
      success: (res) => {
        ...
        for (let i = 0; i < student.length; i++) {  //遍历数据库对象集合
          if (name == student[i].xuehao) { //用户名存在
            if (password !== student[i].password
              ...
              setTimeout(function(){
                wx.navigateTo({
                  url: '/pages/answer/answer',//这里是成功登录后跳转的页面
                })
              },2500);
            }
          } else {   //不存在
